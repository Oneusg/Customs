...
def main():
    ...
    # Apply MPF or HMF based on total value
    mpf = calculate_mpf(total_entered_value) if transport_method == "Air" else 0.0
    ...
    if transport_method == "Air":
        st.write(f"## 📦 MPF (Air, based on total value): ${mpf:,.2f}")
    elif transport_method == "Ocean":
        st.write(f"## ⚓ HMF (Ocean, based on total value): ${hmf:,.2f}")
