...
def main():
    ...
    if hts_input:
        ...
        if not matched_hts.empty:
            ...
            shipment_value = st.number_input("Enter Shipment Value (USD):", min_value=0.0, value=10000.0, step=100.0)
            total_duty_amount = (total_duty / 100) * shipment_value

            st.write(f"### **Total Estimated Duty: {total_duty:.2f}%**")
            st.write(f"### **Estimated Duty Amount: ${total_duty_amount:,.2f} USD** (based on entered shipment value)")
            st.caption("Note: Calculations exclude unit- or quantity-based duties and are for reference only.")
        else:
